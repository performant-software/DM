 ...
 * 
 */
goog.provide('jquery.plugins.radmenu');
;(function($){
	...
	$.fn.radmenu = function(input, param){
		...
		//hack - removed troublesome try/catch...
		//try {
		...
		/*
		//removed remainder of the try-catch here:
		} catch (e) { 
	...
	var MENU = {
		...
		show: function(evt, fn){ // fn = user input onshow
			...
			//var $radialMenu = $(container.html);//HACK
			//$radialMenu.addClass(container.clz);
			//$radialMenu.css(container.css);
		...
		select: function(evt, selectIndex){
			...
			//HACK: Modified to allow cancelling by returning false...!
			//also other stuff..lol!
			var result;
			result = $m.opts.onSelect($selected, selectIndex);//HACK - modified...
			//result = !!result;
			result = (result !== false);
			if (result)
			{
			...
			//$m.opts.onSelect($selected, selectIndex);//HACK - modified...
	...
	function getCoords(iIdx, iNum, oOpts, bClockwise){
		...
		//hack -- begin fixed count hack
		var undef;
		
		//new property - 
		var minCount = oOpts.minSlices; //Min slices - sets a lower bound on how many entries to "pretend" there are, for layout purposes...!
		//if (min
		if ((minCount == undef)||(minCount==null))
		{//^lolcan't use referenceutil up there... =/
		}
		else
		{
			minCount = 0 + minCount;//lol!
			if (minCount <= 0)
			{
				//minSlices 
			}
