...
sc.canvas.FabricCanvas.createFromResource = function(resource) {
...
sc.canvas.FabricCanvas.createFromUri = function(uri, databroker) {
    return sc.canvas.FabricCanvas.createFromResource(
        databroker.getResource(uri)
    );
};

sc.canvas.FabricCanvas.createFromDeferredResource = function(deferred, callback) {
 ...
 */
sc.canvas.FabricCanvas.createDeferredCanvas = function(uri, databroker, opt_urisInOrder, opt_index, opt_doAfter) {
    ...
    var withResource = function(resource) {
        if (!(resource.hasAnyPredicate(sc.canvas.FabricCanvas.RDF_ENUM.width) &&
            ...
            canvas.findAndAddImages();
            canvas.findAndAddSegments();
            canvas.findAndAddConstraints();
            canvas.findAndAddComments();
...
sc.canvas.FabricCanvas.prototype.findAndAddImages = function() {
    ...
    for (var i = 0, len = imageUris.length; i < len; i++) {
        ...
        if (imageResource.hasAnyType(sc.canvas.FabricCanvas.RDF_ENUM.imageTypes)) {
            ...
            this.addImage(databroker.getImageSrc(imageUri), this.getSize());
        }
    }
};

sc.canvas.FabricCanvas.prototype.findAndAddComments = function() {
    var databroker = this.databroker;
...
sc.canvas.FabricCanvas.prototype.findAndAddConstraints = function() {
    var databroker = this.databroker;

    var constraintUris = databroker.getUrisWithProperty(
        sc.canvas.FabricCanvas.RDF_ENUM.constrains, '<' + this.uri + '>'
    );
...
sc.canvas.FabricCanvas.prototype.findAndAddSegments = function() {
    var databroker = this.databroker;
