goog.provide('dm.data.SyncService');

goog.require('dm.data.ConjunctiveQuadStore');
 ...
 */
dm.data.SyncService = function(options) {
    this.databroker = null; // Should be set by owning databroker

    this.options = {};
    goog.object.extend(this.options, dm.data.SyncService.DEFAULT_OPTIONS, options || {});
};

dm.data.SyncService.DEFAULT_OPTIONS = {
...
dm.data.SyncService.prototype.annotsDeleted = function(deleted) {
   var currentProject = this.databroker.projectController.currentProject;
   var url = this.restUrl(currentProject.uri, dm.data.SyncService.RESTYPE.project, null, null) + 'removed';
   $.ajax({
      url : url,
      method : "POST",
...
dm.data.SyncService.prototype.requestSync = function() {
   this.postNewResources();
   this.putModifiedResources();
   this.deleteDeletedResources();
};

dm.data.SyncService.prototype.requestSyncNew = function() {
    this.postNewResources();
};

dm.data.SyncService.prototype.createTextHttpUri = function() {
...
dm.data.SyncService.prototype._restUri = function(baseUri, projectUri, resType, resUri, params) {
    ...
    if (resType == dm.data.SyncService.RESTYPE.text) {
        ...
        url += "/";
    } else if (resType == dm.data.SyncService.RESTYPE.annotation) {
        url += this.options.restAnnotationPath.replace(/^\/+|\/+$/g, "");
        url += "/";
    } else if (resType == dm.data.SyncService.RESTYPE.user) {
        url += this.options.restUserPath.replace(/^\/+|\/+$/g, "");
        url += "/";
    }
    else if (resType == dm.data.SyncService.RESTYPE.project) {
        // pass
    }
    else if (resType == dm.data.SyncService.RESTYPE.search) {
...
dm.data.SyncService.prototype.restUrl = function(projectUri,resType, resUri, params) {
...
dm.data.SyncService.prototype.restUri = function(projectUri, resType, resUri, params) {
    return this._restUri(this.options.dmBaseUri, projectUri, resType, resUri, params);
};


dm.data.SyncService.prototype.getModifiedResourceUris = function() {
...
dm.data.SyncService.prototype.getModifiedResourceUriCount = function() {
...
dm.data.SyncService.prototype.postNewResources = function() {
    var conjunctiveStore = new dm.data.ConjunctiveQuadStore([this.databroker.quadStore, this.databroker.deletedQuadsStore]);
    var graph = new dm.data.Graph(conjunctiveStore, null);

    var urisToRemove = new goog.structs.Set();

    goog.structs.forEach(this.databroker.newResourceUris, function(uri) {
        var conjunctiveResource = new dm.data.Resource(this.databroker, graph, uri);
...
dm.data.SyncService.prototype.putModifiedResources = function() {
    var conjunctiveStore = new dm.data.ConjunctiveQuadStore([this.databroker.quadStore, this.databroker.deletedQuadsStore]);
    var graph = new dm.data.Graph(conjunctiveStore, null);
...
dm.data.SyncService.prototype.deleteDeletedResources = function() {
    var quadsToRemove = [];

    goog.structs.forEach(this.databroker.deletedResourceUris, function(uri) {
        quadsToRemove = quadsToRemove.concat(this.databroker.deletedQuadsStore.query(dm.data.Term.wrapUri(uri), null, null, null));
    });
    ...
    if (quadsToRemove.length > 0) {
        var currentProject = this.databroker.projectController.currentProject;
        var url = this.restUrl(currentProject.uri, dm.data.SyncService.RESTYPE.project, null, null) + 'remove_triples';
...
dm.data.SyncService.prototype.sendResource = function(uri, method, successHandler) {
    var resource = this.databroker.getResource(uri);
    ...
    var PERMISSIONS = dm.data.ProjectController.PERMISSIONS;
    var VOCABULARY = dm.data.DataModel.VOCABULARY;
    ...
    if (conjunctiveResource.hasAnyType(VOCABULARY.textTypes)) {
        resType = dm.data.SyncService.RESTYPE.text;
        ...
        var resType = dm.data.SyncService.RESTYPE.project;
        ...
        resType = dm.data.SyncService.RESTYPE.user;
...
dm.data.SyncService.prototype.hasUnsavedChanges = function() {
...
dm.data.SyncService.prototype.getProjectDownloadUrl = function(projectUri) {
    return this.restUrl(projectUri, dm.data.SyncService.RESTYPE.project, null, null) + 'download';
};
