goog.provide('dm.canvas.FabricCanvasFactory');

goog.require('dm.canvas.FabricCanvas');
goog.require('goog.array');

dm.canvas.FabricCanvasFactory.createFromResource = function(resource) {
    ...
    var canvas = new dm.canvas.FabricCanvas(uri, databroker, size);

    return canvas;
};

dm.canvas.FabricCanvasFactory.createFromUri = function(uri, databroker) {
    return dm.canvas.FabricCanvas.createFromResource(
        databroker.getResource(uri)
    );
};

dm.canvas.FabricCanvasFactory.createFromDeferredResource = function(deferred, callback) {
    deferred.done(function() {
        var canvas = dm.canvas.FabricCanvas.createFromResource(this);
 ...
 * @param {dm.data.Databroker} databroker a databroker which can be queried for
 * information about the canvas.
 * @param {Object|dm.util.Size} displaySize the initial screen display size of
 * the entire canvas.
 ...
 */
dm.canvas.FabricCanvasFactory.createDeferredCanvas = function(uri, databroker, opt_urisInOrder, opt_index, opt_doAfter) {
    ...
    var withResource = function(resource) {
        if (
                !(
                    resource.hasAnyPredicate(dm.canvas.FabricCanvas.RDF_ENUM.width) &&
                    resource.hasAnyPredicate(dm.canvas.FabricCanvas.RDF_ENUM.height)
                ) &&
            ...
            if (! canvas) {
                var width = Number(resource.getOneProperty(
                    dm.canvas.FabricCanvas.RDF_ENUM.width
                ));
                var height = Number(resource.getOneProperty(
                    dm.canvas.FabricCanvas.RDF_ENUM.height
                ));
                ...
                canvas = new dm.canvas.FabricCanvas(
                    uri,
                    databroker,
                    actualSize
                );
            ...
            // setTimeout(function() {
                dm.canvas.FabricCanvasFactory.findAndAddImages(canvas);

            // setTimeout(function() {
                dm.canvas.FabricCanvasFactory.findAndAddSegments(canvas);

            // setTimeout(function() {
                dm.canvas.FabricCanvasFactory.findAndAddSelectors(canvas);
                // dm.canvas.FabricCanvasFactory.findAndAddComments(canvas);
                dm.canvas.FabricCanvasFactory.findAndAddTranscriptions(canvas);
...
dm.canvas.FabricCanvasFactory.findAndAddImages = function(canvas) {
    ...
    for (var i = 0, len = imageAnnoUris.length; i < len; i++) {
        ...
        imageUris = imageUris.concat(
            imageAnnoResource.getProperties(dm.canvas.FabricCanvas.RDF_ENUM.hasBody)
        );
    }
    ...
    for (var i = 0, len = imageUris.length; i < len; i++) {
        ...
        if (imageResource.hasAnyType(dm.data.DataModel.VOCABULARY.imageTypes)) {
            if (! canvas.imagesBySrc.containsKey(imageUri)) {
                canvas.addImageResource(imageResource);
            }
        }
        else if (imageResource.hasAnyType(dm.data.DataModel.VOCABULARY.imageChoiceTypes)) {
            var optionUris = imageResource.getProperties(
                dm.canvas.FabricCanvas.RDF_ENUM.option
            );
...
dm.canvas.FabricCanvasFactory.findAndAddComments = function(canvas) {
    var databroker = canvas.databroker;

    var annoUris = databroker.dataModel.findAnnosReferencingResourceAsBody(
        canvas.uri,
        dm.canvas.FabricCanvas.RDF_ENUM.commentAnno
    );
    ...
    for (var i = 0, len = annoUris.length; i < len; i++) {
        ...
        var bodyUri = annoResource.getOneProperty(dm.canvas.FabricCanvas.RDF_ENUM.
                                                  hasBody);
        var bodyResource = databroker.getResource(bodyUri);

        var content = bodyResource.getOneProperty(dm.canvas.FabricCanvas.RDF_ENUM.
                                                  cntChars);
    }
};

dm.canvas.FabricCanvasFactory.findAndAddSelectors = function(canvas) {
    var databroker = canvas.databroker;
...
dm.canvas.FabricCanvasFactory.findAndAddSegments = function(canvas) {
    var databroker = canvas.databroker;
    ...
    for (var i = 0, len = partUris.length; i < len; i++) {
        ...
        if (! canvas.segmentUris.contains(partUri)) {
            var constraintAttrs = dm.data.DataModel.getConstraintAttrsFromUri(
                                                                       partUri);
            ...
            for (var j = 0, lenj = annoUris.length; j < lenj; j++) {
                ...
                if (annoResource.hasAnyType(dm.canvas.FabricCanvas.RDF_ENUM.
                                            textAnno)) {
                    addedTextUris = addedTextUris.concat(
                        dm.canvas.FabricCanvasFactory.addTextAnnotation(canvas, annoResource, constraintAttrs)
                    );
                }
                else if (annoResource.hasAnyType(
                                        dm.canvas.FabricCanvas.RDF_ENUM.imageAnno)) {
                    // canvas.addImageAnnoSegment(annoResource, constraintAttrs);
                }
                else if (annoResource.hasAnyType(
                                        dm.canvas.FabricCanvas.RDF_ENUM.audioAnno)) {
                    dm.canvas.FabricCanvasFactory.addAudioAnno(canvas, annoResource, constraintAttrs);
                }
                else {
                }
            }
...
dm.canvas.FabricCanvasFactory.findAndAddTranscriptions = function(canvas) {
    var databroker = canvas.databroker;
            ...
            if (annotationsListUri) {
                goog.structs.forEach(databroker.getListUrisInOrder(annotationsListUri), function(annoUri) {
                    ...
                    if (anno.hasProperty('oa:motivatedBy', 'sc:painting') && goog.string.startsWith(targetUri, [canvasResource.uri, '#'].join(''))) {
                        var constraintAttrs = dm.data.DataModel.getConstraintAttrsFromUri(targetUri);
                        dm.canvas.FabricCanvasFactory.addTextAnnotation(canvas, anno, constraintAttrs);
                    }
                });
            }
        });
...
dm.canvas.FabricCanvasFactory.addTextAnnotation = function(canvas, annoResource, constraintAttrs) {
    var addedTextUris = [];
    var databroker = annoResource.getDatabroker();

    var bodyUris = annoResource.getProperties(dm.canvas.FabricCanvas.RDF_ENUM.hasBody);
    for (var k = 0, lenk = bodyUris.length; k < lenk; k++) {
        ...
        var text = null;
        if (bodyResource.hasAnyPredicate(dm.canvas.FabricCanvas.RDF_ENUM.cntChars)) {
            text = bodyResource.getOneProperty(
                dm.canvas.FabricCanvas.RDF_ENUM.cntChars);
        } else if (
            bodyResource.hasAnyPredicate(dm.canvas.FabricCanvas.RDF_ENUM.cnt08Chars)
        ){
            text = bodyResource.getOneProperty(
                dm.canvas.FabricCanvas.RDF_ENUM.cnt08Chars);
        }
...
dm.canvas.FabricCanvasFactory.addAudioAnno = function(canvas, annoResource, constraintAttrs) {
