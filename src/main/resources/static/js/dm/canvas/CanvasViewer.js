goog.provide('dm.canvas.CanvasViewer');

goog.require('goog.dom');
goog.require('goog.math.Size');
goog.require('goog.events');

goog.require('dm.canvas.CanvasToolbar');
goog.require('dm.canvas.DragFeatureControl');
goog.require('dm.canvas.ZoomSliderControl');
 ...
 */
dm.canvas.CanvasViewer = function(options) {
    ...
    this.mainViewport = new dm.canvas.FabricCanvasViewport(this.databroker);
    this.marqueeViewport = new dm.canvas.FabricCanvasViewport(this.databroker);

    this.toolbar = new dm.canvas.CanvasToolbar(this);
    this.setupControls();
...
dm.canvas.CanvasViewer.prototype.options = {
    'marqueeSize': new goog.math.Size(100, 100)
};

dm.canvas.CanvasViewer.prototype.isEditable = function() {
    return this._isEditable;
};

dm.canvas.CanvasViewer.prototype.makeEditable = function() {
...
dm.canvas.CanvasViewer.prototype.makeUneditable = function() {
    if (this.isEditable()) {
...
dm.canvas.CanvasViewer.prototype.setupControls = function() {
    this.zoomSlider = new dm.canvas.ZoomSliderControl(this.mainViewport);
    this.zoomSlider.activate();
};

dm.canvas.CanvasViewer.prototype.render = function(div) {
    div.appendChild(goog.dom.getElement(this.baseDiv));
};

dm.canvas.CanvasViewer.prototype.getElement = function() {
    return this.baseDiv;
};

dm.canvas.CanvasViewer.prototype.resize = function(width, height) {
   if (height == null) {
      height = width.height;
      width = width.width;
   }
...
dm.canvas.CanvasViewer.prototype.getDisplaySize = function() {
    var width = jQuery(this.viewportDiv).width();
    var height = jQuery(this.viewportDiv).height();
    
    return new goog.math.Size(width, height);
};
dm.canvas.CanvasViewer.prototype.getSize =
dm.canvas.CanvasViewer.prototype.getDisplaySize;

dm.canvas.CanvasViewer.MARQUEE_MARKER_SHOWN_OPACITY = 0.6;
dm.canvas.CanvasViewer.MARQUEE_MARKER_HIDDEN_OPACITY = 0.4;

dm.canvas.CanvasViewer.prototype._adjustMarqueeFeatureStyles = function() {
    ...
    goog.structs.forEach(marqueeCanvas.objectsByUri, function(obj, uri) {
        if (dm.canvas.FabricCanvas.MARKER_TYPES.contains(obj.type)) {
            ...
            if (mainObject && mainObject.visible === true) {
                obj.set('opacity', dm.canvas.CanvasViewer.MARQUEE_MARKER_SHOWN_OPACITY);
            }
            else {
                obj.set('opacity', dm.canvas.CanvasViewer.MARQUEE_MARKER_HIDDEN_OPACITY);
            }
        }
    }, this);
...
dm.canvas.CanvasViewer.prototype.addDeferredCanvas = function(deferred) {
    var self = this;
    var _canvas = null;
...
dm.canvas.CanvasViewer.prototype.setCanvas = function(canvas) {
    this.mainViewport.clear();
    this.marqueeViewport.clear();

    this.mainViewport.setCanvas(canvas);

    var deferredMarqueeCanvas = dm.canvas.FabricCanvasFactory.createDeferredCanvas(
        canvas.getUri(),
        this.databroker,
    ...
    deferredMarqueeCanvas.done(function (marqueeCanvas) {
        canvas.addEventListener(
            ...
            function(event) {
                dm.canvas.FabricCanvasFactory.findAndAddSelectors(marqueeCanvas);
...
dm.canvas.CanvasViewer.prototype.handleMarqueeMousedown = function(event) {
...
dm.canvas.CanvasViewer.prototype.handleMarqueeMousemove = function(event) {
...
dm.canvas.CanvasViewer.prototype.handleMarqueeMouseup = function(event) {
    this.isMarqueeDragging = false;

    jQuery(this.marqueeViewport.getElement()).removeClass('sc-CanvasViewport-drag');
};
dm.canvas.CanvasViewer.prototype.handleMarqueeMousewheel = function(event, delta, deltaX, deltaY) {
    if (this.mainViewport.isEmpty()) {
        return;
    }
...
dm.canvas.CanvasViewer.prototype.updateMarqueeBox = function() {
    ...
    if (this.marqueeBox && this.mainViewport.canvas && this.marqueeViewport.canvas) {
        ...
        var centeredCoords = dm.canvas.FabricCanvas.toCenteredCoords(topLeft.x, topLeft.y, width, height);
...
dm.canvas.CanvasViewer.prototype.requestFrameRender = function() {
