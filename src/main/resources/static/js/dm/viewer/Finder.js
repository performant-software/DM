...
goog.require('dm.resource.MarkerAnnotationSummary');
goog.require('dm.resource.MarkerCollection');
goog.require('dm.ui.PopupWindow');
goog.require('dm.widgets.DialogWidget');
...
/**
 * @extends {dm.viewer.ResourceListViewer}
 ...
 */
dm.viewer.Finder = function (clientApp, opt_contextId, opt_contextType) {
    dm.viewer.ResourceListViewer.call(this, clientApp);
...
goog.inherits(dm.viewer.Finder, dm.viewer.ResourceListViewer);

dm.viewer.Finder.VIEWER_TYPE = 'finder';

dm.viewer.Finder.prototype.setContextType = function (contextType) {
    this.contextType = contextType;
};

dm.viewer.Finder.prototype.getContextType = function () {
 ...
 */
dm.viewer.Finder.prototype.loadSummaries = function (ids, opt_queryText) {
    ...
    var withIds = function (ids) {
        ...
        if (this.getContextType() == dm.viewer.Finder.ContextTypes.RESOURCE && 
...
//        }
//        
//        var dummyResource = dm.resource.ResourceFactory.createFromJSON(dummyData);
//        
...
//        this.addResourceSummaries(summaries);
    ...
    if (this.getContextType() == dm.viewer.Finder.ContextTypes.SEARCH) {
        this.crawler.crawlForSearch(ids, this.queryText, withIds, this, dm.Util.scopeAsyncHandler(this.flashErrorIcon, this));
...
//        this.crawler.crawl(ids, this.queryText, withIds, this, undefined, this.flashErrorIcon);
        ...
        this.crawler.crawl(ids, this.queryText, withIds, this, this.contextId, dm.Util.scopeAsyncHandler(this.flashErrorIcon, this));
    }

};

dm.viewer.Finder.prototype.autoLoadResourceFromAnno = function () {
    ...
    if (annos.length == 1 && this.getContextType() == dm.viewer.Finder.ContextTypes.RESOURCE) {
...
dm.viewer.Finder.prototype.showEmptyMessage = function () {
    var message;
    if (this.getContextType() == dm.viewer.Finder.ContextTypes.SEARCH) {
 ...
 */
dm.viewer.Finder.prototype.setLoading = function (isLoading) {
    if (isLoading) {
        this.loading = true;
 ...
 * @return {boolean}
 */
dm.viewer.Finder.prototype.isLoading = function () {
    return this.loading;
};
 ...
 * @return {Array.<dm.resource.ResourceSummary>}
 */
dm.viewer.Finder.prototype.createSummariesFromResource = function (resource) {
    var summaries = [];
    ...
    if (resource.getType() == 'anno') {
        ...
        var summary = dm.resource.ResourceSummaryFactory.createFromResource(
            resource,
            dm.Util.scopeAsyncHandler(this.handleResourceSummaryClick),
            this,
            this.clientApp,
            this.domHelper
        );
        summary.setDeletableResources([resource.id]);
        summary.setView(dm.resource.RESOURCE_VIEW);
...
dm.viewer.Finder.prototype.createSummariesFromResources = function (resources) {
    var summaries = [];
 ...
 * @return {Array.<dm.resource.ResourceSummary>}
 */
dm.viewer.Finder.prototype.createAnnoSummariesFromResource_ = function (resource) {
...
dm.viewer.Finder.prototype.createAnnoSummariesFromResourceIds_ = function (bodyId, targetIds, annoIds, triplesByTarget) {
    var summaries = [];
    ...
    var createTargetSummaries = function (targetsIds, opt_createMarkerCollections) {
        ...
        var setTargetSummaryProperties = function (summary) {
            ...
            summary.setView(dm.resource.TARGET_VIEW);
    ...
    if (this.getContextType() == dm.viewer.Finder.ContextTypes.SEARCH || bodyId != this.contextId) {
        var numChildren = 0;
        
        if (bodyResource) {
            if (bodyResource.getType() == 'marker') {
                summary = new dm.resource.MarkerAnnotationSummary(null, null, this, annoIds[0]);
            }
            else {
                summary = new dm.resource.AnnotationSummary(null, null, this, annoIds[0]);
            ...
            bodySummary.setView(dm.resource.BODY_VIEW);
...
dm.viewer.Finder.prototype.createHighlightAnnoSummaries = function (annos) {
    var summaries = [];
 ...
 * @param opt_withMarkerSummaries {Function(dm.resource.ResourceSummary)}
 * @param opt_scope {Object}
 * @return {dm.resource.MarkerCollection}
 */
dm.viewer.Finder.prototype.createMarkerCollections = function (markers, opt_hideCanvasNames, opt_withMarkerSummaries, opt_scope) {
    ...
    for (var canvasId in markersByCanvasId) {
        ...
        var markerCollection = new dm.resource.MarkerCollection(canvasId, title, dm.Util.scopeAsyncHandler(this.handleMarkerCollectionClick, this), this, this.domHelper);
...
dm.viewer.Finder.prototype.setupEventHandlers = function () {
...
dm.viewer.Finder.prototype.render = function () {
    if (this.rootDiv != null) {
        return;
    }
    
    dm.viewer.ResourceListViewer.prototype.render.call(this);
...
dm.viewer.Finder.prototype.finishRender = function () {
    dm.viewer.ResourceListViewer.prototype.finishRender.call(this);
...
dm.viewer.Finder.prototype.setQueryText_ = function(queryText) {
    this.queryText = queryText;
    this.searchField.value = queryText;
};


dm.viewer.Finder.prototype.refresh = function (e) {
    if (this.isLoading()) {
        return;
    }
 ...
 * dm.resource.ResourceSummary.HANDLER_MSG
 * @param handler {function(dm.viewer.PanelContainer)}
 * @param opt_scope {Object=} the scope in which to call doAfter
 */
dm.viewer.Finder.prototype.withAppropriatePanel = function (findId, event, message, handler, opt_scope) {
    ...
    (message == dm.resource.ResourceSummary.HANDLER_MSG.swapPanels);
    var newWindow = (message == dm.resource.ResourceSummary.HANDLER_MSG.newWindow);
 ...
 *
 *@param handler {Function({dm.viewer.PanelContainer}, {dm.ui.PopupWindow})}
 *@param opt_scope {Object}
 */
dm.viewer.Finder.prototype.openPopoutPanel = function (handler, opt_scope) {
    var location = this.clientApp.getWebService().popoutURI();
    
    var popup = new dm.ui.PopupWindow(
        location,
        {
            'width': 570,
            'height': 670
...
dm.viewer.Finder.prototype.onPopoutLoaded = function (event, popup, doAfter, opt_scope) {
    ...
    var popupPanel = new dm.viewer.PanelContainer(name, newDiv, newTab, null, popupWin.document);
 ...
 *
 * @param id {string}
 * @param summary {dm.resource.ResourceSummary}
 * @param event {goog.events.Event}
 * @param opt_params {Object}
 */
dm.viewer.Finder.prototype.handleResourceSummaryClick = function (id, summary, event, opt_params) {
    var resource = summary.resource;
    var type = resource.getType();
    
    var fireClickEvent = function () {
        var eventDispatcher = this.clientApp.getEventDispatcher();
        var event = new dm.events.ResourceClick(resource.getId(), eventDispatcher, this);
        eventDispatcher.dispatchEvent(event);
    };
    fireClickEvent = dm.Util.scopeAsyncHandler(fireClickEvent, this);
 ...
 * @param summary {dm.resource.ResourceSummary}
 * @param event {goog.events.Event}
 * @param message {string} from an enumeration of dm.resource.ResourceSummary.HANDLER_MSG
 */
dm.viewer.Finder.prototype.handleMarkerCollectionClick = function (id, summary, event, message) {
 ...
 * @param panel <dm.viewer.PanelContainer>
 */
dm.viewer.Finder.prototype.loadText = function (id, title, panel) {
    if (panel.getViewer() && panel.getViewer().resourceId == id) {
        return;
    }
    
    var viewer = new dm.viewer.TextEditor(this.clientApp, null, null);
 ...
 * @param panel <dm.viewer.PanelContainer>
 */
dm.viewer.Finder.prototype.loadTextHighlight = function (id, textId, title, panel) {
    var viewer = panel.getViewer();
    ...
    if (viewer && viewer.resourceId == textId) {
        ...
        var newViewer = new dm.viewer.TextEditor(this.clientApp, null, null);
 ...
 * @param panel {dm.viewer.PanelContainer}
 */
dm.viewer.Finder.prototype.loadCanvas = function (id, panel) {
	var viewer = new dm.viewer.StandardSimpleMarkerEditor(this.clientApp,null,null,panel.getDomHelper());
 ...
 * @param resource {dm.resource.MarkerResource}
 * @param panel {dm.viewer.PanelContainer}
 * @param zoomIn {boolean} true if the viewer should zoom in to the specified marker, false if not
 */
dm.viewer.Finder.prototype.toggleMarker = function (id, canvasId, resource, panel, zoomIn) {
    var viewer = panel.getViewer();
...
dm.viewer.Finder.prototype.loadMarkerInternal_ = function (id, resource, viewer) {
 ...
 * @param zoomIn {boolean} true if the viewer should zoom in to the specified marker, false if not
 */
dm.viewer.Finder.prototype.loadMarker = function (id, canvasId, resource, panel, zoomIn) {
    var viewer = panel.getViewer();
    ...
    if (viewer && viewer.resourceId == canvasId) {
        ...
        var newViewer = new dm.viewer.StandardSimpleMarkerEditor(this.clientApp,null,null,panel.getDomHelper());
 ...
 * @param resourceArr {Array.<dm.resource.MarkerResource>} an array of marker resources
 * @param panel {dm.viewer.PanelContainer}
 */
dm.viewer.Finder.prototype.loadMarkers = function (canvasId, resourceArr, panel) {
    var viewer = panel.getViewer();
 ...
 * @param resource {dm.resource.MarkerResource}
 */
dm.viewer.Finder.prototype.unloadMarker = function (resource) {
 ...
 */
dm.viewer.Finder.prototype.handleDeleteEnableClick_ = function (e) {
    e.stopPropagation();
 ...
 */
dm.viewer.Finder.prototype.enableDelete = function () {
    ...
    this.forEachSummary(function (summary) {
        if (this.hasDeletePermissions(summary)) {
            summary.enableDelete(dm.Util.scopeAsyncHandler(this.summaryDeleteHandler, this));
        }
    }, this);
};
 ...
 */
dm.viewer.Finder.prototype.disableDelete = function () {
 ...
 */
dm.viewer.Finder.prototype.hasDeletePermissions = function (summary) {
...
/**
 * Called by a {dm.resource.ResourceSummary} when its delete button is clicked
 * 
 * @param summary {dm.resource.ResourceSummary}
 * @param event {goog.events.Event}
 */
dm.viewer.Finder.prototype.summaryDeleteHandler = function (summary, event) {
...
dm.viewer.Finder.prototype.deleteOrUnlinkSummary = function (summary) {
    var resource = summary.resource;
    
    var ids = summary.getDeletableResources();
    this.deleteResources(ids);
    if (summary.getView() == dm.resource.TARGET_VIEW) {
        this.visualUnlinkAndMoveSummary(summary);
    }
    else if (summary.getView() == dm.resource.BODY_VIEW) {
...
dm.viewer.Finder.prototype.deleteResources = function (ids) {
 ...
 * @param summary {dm.resource.ResourceSummary}
 */
dm.viewer.Finder.prototype.displayDeleteConfirmDialog = function (summary) {
    var summaryView = summary.getView();
    var isInAnnotation = summaryView == dm.resource.TARGET_VIEW || summaryView == dm.resource.BODY_VIEW;
    ...
    if (isInAnnotation) {
        if (summary.getView() == dm.resource.BODY_VIEW) {
    ...
    var confirmDialog = new dm.widgets.DialogWidget({
        bModal: true,
        caption: caption,
        content: message,
        show_buttons: [
 ...
 */
dm.viewer.Finder.prototype.visualDeleteSummary = function (summary, opt_unpaintAllMarkers, opt_deleteAllOccurrences) {
    this.visualDeleteSummaryInternal_(summary);
...
dm.viewer.Finder.prototype.visualUnlinkAndMoveSummary = function (summary) {
    this.visualDeleteSummaryInternal_(summary);
};

dm.viewer.Finder.prototype.addResourceSummary = function (resourceSummary) {
    dm.viewer.ResourceListViewer.prototype.addResourceSummary.call(this, resourceSummary);
 ...
 */
dm.viewer.Finder.prototype.syncSelectedMarkersWithViewer = function (viewer) {
...
dm.viewer.Finder.prototype.syncSelectedMarkersWithAppropriateViewer = function (opt_doAfter, opt_scope) {
    if (!this.contextId) {
        return;
    }
...
goog.provide('dm.viewer.Finder.ContextTypes');
dm.viewer.Finder.ContextTypes.RESOURCE = 'resource';
dm.viewer.Finder.ContextTypes.SEARCH = 'search';