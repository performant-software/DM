goog.provide("dm.util.DomTraverser");
goog.require("dm.util.Set");
goog.require("dm.util.Map");

//dm.util.DomTraverser = function(commonAncestor, start, end)
dm.util.DomTraverser = function(start, end)
{
...
dm.util.DomTraverser.prototype.fixSearchResultsHack_ = function()
{
	//debugPrint("!!");
	//return;
	//debugPrint("!!!2");
	...
	for(var i=0,l=tmpArray.length; i<l; i++)
	{
		...
		//if (nd.firstChild !== null)
		{
			...
			while(testNd !== null)
			{
				if (!dm.util.DomTraverser.hasNodeHelper_(tmpArray, testNd))
				{
					bFoundIt=false;
					break;
				}
...
//dm.util.DomTraverser.prototype.findFirstAncestorBefore(node, beforeAncestor)
//dm.util.DomTraverser.prototype.findAncestorChainBefore(node, beforeAncestor)
//dm.util.DomTraverser.prototype.findAnce
dm.util.DomTraverser.prototype.findAncestorChainBefore=function(node, beforeAncestor)
{
	var ret = [];
...
dm.util.DomTraverser.prototype.findCommonAncestor = function(nodeA, nodeB)
{
...
dm.util.DomTraverser.prototype.findFirstAncestorBefore=function(node, before)
{
...
dm.util.DomTraverser.prototype.performTheSearch_ = function(atNode)
{
	if (atNode === null)
	{
		return;
...
dm.util.DomTraverser.prototype.getTraversal = function()
{
	return this.results;
};

dm.util.DomTraverser.prototype.each = function(callback)//function()
{
	var moreArgs = [];
...
//dm.util.DomTraverser.prototype.generateMinimumNodeSequence = function()
//dm.util.DomTraverser.prototype.findInArray = function

dm.util.DomTraverser.prototype.hasNode_= function(find_nd)
{
	
	var rawSeq = this.getTraversal();//.;
	return dm.util.DomTraverser.hasNodeHelper_(rawSeq, find_nd);
};

dm.util.DomTraverser.hasNodeHelper_ = function(rawSeq, find_nd)
{
	var i, l;
...
//dm.util.DomTraverser.prototype.replaceBunchWithCommonParent(bunch, setParent)//parentNode)
dm.util.DomTraverser.prototype.replaceBunchWithCommonParent=function(bunch, setParent)//parentNode)
...
dm.util.DomTraverser.prototype.eachTextNode = function(callback)
{
	var argumentsArr = [];
...
dm.util.DomTraverser.prototype.generateMinimumNodeBunchSequence = function()
{
...
dm.util.DomTraverser.prototype.dbgToPrintableHTMLString=function(htmlString)
...
/*
dm.util.DomTraverser.prototype.traversal = function()
{
