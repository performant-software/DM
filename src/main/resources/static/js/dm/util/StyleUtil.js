goog.provide("dm.util.StyleUtil");
...
**/
//dm.util.StyleUtil = dm.util.StyleUtil; //this line is a hack to allow jsdoc to "find" this 'module'(namespace)...
...
**/
dm.util.StyleUtil.modifyStyleCoordinate = function(currentValue, deltaValue, opt_units)
{
 ...
 * @public
**/
dm.util.StyleUtil.includeStyleSheetOnce = function(cssPath)
{
	//debugPrint("including stylesheet: "+cssPath);//HACK
	
	//Q: is there a better implementation out there... probably...???
	
	var set = dm.util.StyleUtil.includedStyleSheets_;
	if (dm.util.ReferenceUtil.isBadReferenceValue(set[cssPath]))
	{
		set[cssPath] ={
			cssPath: cssPath
		};
 ...
 * @public
**/
dm.util.StyleUtil.includeStyleSheetOnceFromRoot = function(cssRoot, cssPath)
{
	cssRoot = dm.util.ReferenceUtil.applyDefaultValue(cssRoot, dm.util.StyleUtil.DEFAULT_CSS_ROOT);
	//cssPath = "" + cssRoot +"/"+ cssPath;	// "/" -- Nah... use below instead:
	cssPath = "" + cssRoot + cssPath;
	return dm.util.StyleUtil.includeStyleSheetOnce(cssPath);
 ...
 * @private
**/
dm.util.StyleUtil.includedStyleSheets_ = {};
 ...
 * @static
**/
dm.util.StyleUtil.DEFAULT_CSS_ROOT = "http://ada.drew.edu/anno/css/";

dm.util.StyleUtil.CURSOR_SIZE = 16;

dm.util.StyleUtil.maintainPopupPositionWithinWindow = function (initialPosition, div, domHelper) {
...
dm.util.StyleUtil.computeCenteredBottomClientPosition = function (div, opt_domHelper) {
