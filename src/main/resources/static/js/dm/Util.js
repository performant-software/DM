goog.provide('dm.Util');


dm.Util.pipeline = function(f1, f2, opt_f1Scope, opt_f2Scope) {
    f1 = dm.Util.scopeAsyncHandler(f1, opt_f1Scope);
    f2 = dm.Util.scopeAsyncHandler(f2, opt_f2Scope);
...
dm.Util.scopeAsyncHandler = function(handler, opt_scope) {
 ...
 *
 * For code readability, the .bind(this) syntax should be used rather than {dm.Util.scopeAsyncHandler}
 */
 ...
 */
dm.Util.timeoutSequence = function (delayMS, fns, opt_scope) {
    var sumDelays = 0;
    ...
    for (var i=0, len=fns.length; i<len; i++) {
        ...
        window.setTimeout(dm.Util.scopeAsyncHandler(fnI, scopeI), delayI + sumDelays);
 ...
 */
dm.Util.runAsync = function (fn, opt_scope) {
    return window.setTimeout(dm.Util.scopeAsyncHandler(fn, opt_scope), 1);
};