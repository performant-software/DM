...
def handle_user(user):
    ...
    graph.add((project_uri, DC['title'], Literal("Imported Project")))
    graph.add((project_uri, DCTERMS.description, Literal("Created from the contents of your old DM workspace")))

    user_uri = get_user_uri(user)
    
    # Add data about user
    graph.add((user_uri, PERM['hasPermissionOver'], project_uri))
    graph.add((user_uri, PERM.mayUpdate, project_uri))
    graph.add((user_uri, PERM.mayRead, project_uri))
    graph.add((user_uri, PERM.mayDelete, project_uri))
