...
sc.RepoBrowser.prototype.loadRepositoryByUrl = function(url) {
    ...
    this.databroker.getDeferredResource(url).done(function(resource) {
        var repositoryUri = sc.util.Namespaces.stripAngleBrackets(self.databroker.getResourcesDescribedByUrl(url)[0]);

        self.pushTitleToBreadCrumbs(repositoryUri, function() {
            self.loadRepositoryByUrl(url);
        });

        self.addManifestItems(repositoryUri, jQuery.proxy(self.loadCollectionByUri, self), collectionsDiv);
    });
};
